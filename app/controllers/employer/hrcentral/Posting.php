...
class Posting extends Controller
{
    public function index(){
        ...
        $job_post=$conn->query("SELECT DISTINCT job_post.*,degree_name,position,experience_type   FROM `job_post` join degree on job_degree_id=degree.id join job_position on job_position_id=job_position.id join job_experience on job_experience_id=job_experience.id left join job_post_activity on job_id = job_post.id where posted_by_id ='$employer_id' and NOW() < end_date and status ='1'  ORDER BY posted_date")->fetchAll(PDO::FETCH_ASSOC);


        $count_submitted =$conn->query("SELECT job_id,count(job_id) as num_submit FROM `job_post_activity` group by  job_id")->fetchAll(PDO::FETCH_ASSOC);       
